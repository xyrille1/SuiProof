      ...
      <DialogContent className="max-w-4xl w-full p-0 !rounded-3xl shadow-2xl overflow-hidden grid-cols-1 md:grid-cols-2 grid max-h-[90vh]">
        ...
        <div className="md:w-full p-8 overflow-y-auto order-2 md:order-2">
            <div className="flex justify-between items-start mb-6">
                <div>
                    <h2 className="text-2xl font-black italic uppercase tracking-tighter text-foreground">Birth Certificate</h2>
                    <p className="text-xs font-code text-muted-foreground">OID: {manifest.provenanceId}</p>
                </div>
            ...
            <div className="space-y-6">
                <div>
                    <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-4">Object Lineage</h4>
                    <div className="flex items-start gap-4">
                        ...
                        <div className="space-y-4 flex-1">
                            <div className="bg-muted/50 p-3 rounded-xl border">
                                <p className="text-xs font-bold text-foreground">Original Capture</p>
                                <p className="text-[10px] text-muted-foreground">{manifest.metadata.captureDate}</p>
                            </div>
                            <div className="bg-muted/30 p-3 rounded-xl border border-dashed opacity-60">
                                <p className="text-xs font-bold text-foreground">Authorized Edit (Pending)</p>
                                <p className="text-[10px] text-muted-foreground italic">No parent-child links detected.</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-foreground rounded-2xl p-6 text-background">
                    <h4 className="text-[10px] font-bold text-primary/70 uppercase mb-4 tracking-widest">Immutable Metadata</h4>
                    <dl className="grid grid-cols-2 gap-y-4 gap-x-2 text-xs font-code">
                        ...
                        <div className="col-span-2">
                            <dt className="text-muted-foreground/50 mb-1">CONTENT_HASH (BLAKE2b)</dt>
                            <dd className="opacity-80 leading-tight break-all">{manifest.metadata.contentHash}</dd>
                        </div>
                    </dl>
                </div>
            </div>
            ...
            <footer className="mt-8 pt-6 border-t flex gap-2">
                <Button variant="outline" className="flex-1 rounded-xl font-bold">Download Proof JSON</Button>
                <Button className="flex-1 rounded-xl font-bold shadow-lg shadow-primary/20">Share Public URL</Button>
            </footer>
        </div>
      </DialogContent>
    </Dialog>
